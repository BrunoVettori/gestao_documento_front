...
function EditarTipoDocumento({ tipo, refetch }: { tipo: any; refetch: () => void }) {
    const [loading, setLoading] = useState(false);
    const [open, setOpen] = useState(false);

    const [ativo, setAtivo] = useState(tipo.ativo === 1 ? true : false);

    const [empresarial, setEmpresarial] = useState(tipo.empresarial === 1 ? true : false);
    ...
    async function SubmitForm(data: any) {
        ...
        const request = new Request(import.meta.env.VITE_BASE_URL + '/tipo_documento', {
            method: 'PUT',
            credentials: 'include',
    ...
    return (
        ...
        >
            ...
            <DialogContent className="w-[900px] h-fit max-w-[80vw] max-h-[80vh] overflow-scroll">
                ...
                <form id="tipo_form" onSubmit={handleSubmit(SubmitForm)} className="mb-5 mt-3 flex gap-5">
                ...
                <LoadingButton loading={loading ? 'true' : 'false'} form="tipo_form">
                    Editar
                </LoadingButton>
            </DialogContent>
        </Dialog>
