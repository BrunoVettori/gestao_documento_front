...
function Empresa() {
    ...
    const query_empresa = useQuery({
        ...
        queryFn: async () => {
            const request = new Request(import.meta.env.VITE_BASE_URL + `/empresas?id=${id}`, {
                method: 'GET',
                credentials: 'include',
    ...
    useEffect(() => {
        if (!query_empresa.isPending && !query_empresa.isError && query_empresa.data !== undefined) {
    ...
    function NovoDoc(refetch: () => void) {
        return (
            <div className="flex items-center gap-3">
                ...
                <CleckListFuncionario nome={empresa.nome} empresa_id={id} refetching={query_empresa.isRefetching} />
                <CleckListDocs nome={empresa.nome} empresa_id={id} refetching={query_empresa.isRefetching} />
    ...
    async function DonwloadFiles() {
        const request = new Request(import.meta.env.VITE_BASE_URL + `/download_zip?empresa_id=${empresa.id}`, {
            method: 'GET',
            credentials: 'include',
            headers: { 'Content-Type': 'application/json' },
        });
            ...
            <div className="h-full w-full p-5 pl-10 pr-10 overflow-hidden">
                <div className="flex p-5 gap-5 relative bg-system_card_background rounded-xl border-[2px] border-system_border">
                    ...
                    <div className="h-[250px] w-[300px] rounded-lg overflow-clip border-[1px] border-system_gray_300">
                        <img
                            ...
                            src={
                                ...
                                `${empresa.foto_perfil ? '/' + empresa.foto_perfil : '/placeholder_empresa.png'}`
                            }
                            alt="Placeholder"
                        />
                    </div>
                    ...
                    <div className="flex flex-col gap-1 min-h-[200px]">
                        ...
                        <p className="text-3xl font-nunito_light text-system_gray_400">{empresa.cargo}</p>

                        <div className="flex flex-col h-fit gap-x-[200px] flex-wrap w-fit mt-5">
                            <div className="flex flex-col gap-5">
                                <div className="flex gap-5">
                                    <div>
                                        <p className="text-lg font-nunito_light text-system_gray_500">CNPJ:</p>
                                        {empresa.email && (
                                    ...
                                    <div>
                                        <p className="text-lg font-nunito_semibold">{empresa.cnpj}</p>
                                        {empresa.email && <p className="text-lg font-nunito_semibold">{empresa.email}</p>}
                                        <p className="text-lg font-nunito_semibold">{empresa.created_at}</p>
                                        {empresa.endereco && <p className="text-lg font-nunito_semibold">{empresa.endereco}</p>}
                                    </div>
                                </div>
                            </div>
                        </div>
                ...
                <div className="flex flex-wrap mt-5 gap-2 items-center justify-between">
                    ...
                    <Vence30Dias empresa_id={id} refetching={query_empresa.isRefetching} />
                    <Vencidos30Dias empresa_id={id} refetching={query_empresa.isRefetching} />
                    <VencidosMais30Dias empresa_id={id} refetching={query_empresa.isRefetching} />
                </div>

                <div className="bg-system_card_background rounded-xl border-[2px] border-system_border mt-5 p-5 w-full">
                    <DocumentoTable NovoDocumento={NovoDoc} refetch={query_empresa.refetch} empresa_id={id} />
                </div>
            </div>
        );
    }
